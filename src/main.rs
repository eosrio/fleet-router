...
use crate::models::{build_config_db, build_state_db, Server, ServerConfig, ServerConfigDb, ServerStateDb, StaticConfig};
        ...
        match connect_async(endpoint.clone()).await {
            Ok((ws, _)) => {
                ...
                match message {
                    ...
                    Some(Err(e)) => {
                        cprintln!(" ❌  <red>{}:</> Failed to read message! Error: {}",endpoint,e);
                    }
                    Some(Ok(m)) => {
                        cprintln!(" ❌  <red>{}:</> Unexpected Message type: {:?}",endpoint, m.type_id());
                    }
                }
            }
            Err(e) => {
            ...
            Command::new("config")
                ...
                .subcommand(
                    Command::new("init")
                        ...
                        .arg(arg!(<CONFIG_OUT> "New config file")
                            .value_parser(value_parser!(PathBuf)))
                )
                .subcommand(
                    Command::new("test")
                        ...
                        .arg_required_else_help(true)
                        .arg(arg!(<CONFIG> "config file to test")
                            .value_parser(value_parser!(PathBuf)))
                )
        );
        ...
        Some(("config", config)) => {
            match config.subcommand() {
                Some(("init", init)) => {
                    ...
                    cprintln!("Creating new config file: <bright-cyan>{:?}</>", path.canonicalize().unwrap());
                ...
                Some(("test", test)) => {
                    match test.get_one::<PathBuf>("CONFIG") {
                        None => {
